...
HRESULT InitPlayer(void)
{
	...
	for (int i = 0; i < PLAYER_PARTS_MAX; i++)
	{
		...
		g_Player[i].anim_use = 0;		// 移動スピードクリア
		g_Player[i].old_anim_use = 0;		// 移動スピードクリア
...
void UpdatePlayer(void)
{
	...
	{
		g_Player[PLAYER_PARENT].anim_use = 2;
	}

	//スライディング処理
	else if (GetKeyboardTrigger(DIK_K))
	{
		g_Player[PLAYER_PARENT].anim_use = 3;
	}
	...
	case 1:	// 下向き
		roty = 0.0f;
		g_Player[PLAYER_PARENT].anim_use = 1;
		break;
	...
	for (int i = 0; i < PLAYER_PARTS_MAX; i++)
	{
		...
		if (g_Player[i].use == true)
		{
			...
			if (g_Player[i].anim_use == 1)
			{
				g_Player[i].tbl_adr = parts_dash_adr[i];
			}

			else if (g_Player[i].anim_use == 2)
			{
				g_Player[i].tbl_adr = parts_jump_adr[i];
			}

			else if (g_Player[i].anim_use == 3)
			{
	...
	if (kiseki_idx > 0)
	{
		if (g_Player[PLAYER_PARENT].anim_use == 1 || g_Player[PLAYER_PARENT].anim_use == 2)
		{
